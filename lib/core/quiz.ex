defmodule Mastery.Core.Quiz do
  ...
  def answer_question(quiz, %Response{correct: true} = response) do
    new_quiz = quiz |> inc_record |> save_response(response)
  ...
  defp select_a_random_question(quiz) do
    quiz.templates
    # gets a random category in the form {category_name, templates}
    |> Enum.random()
    # grabs just the templates
    |> elem(1)
    # pick a random template
    |> Enum.random()
    # create a new question from that template
    |> Question.new()
  end
